...
GethApiDouble.prototype.eth_gasPrice = function(address, callback) {
  // hack taken from miner_start
  if (!callback && typeof address === "function") {
    callback = address;
    address = null;
  }

  callback(null, utils.addHexPrefix(this.state.gasPrice(address)));
};

// indicate that `eth_gasPrice` only requires one argument (the callback)
GethApiDouble.prototype.eth_gasPrice.minLength = 1;
...
GethApiDouble.prototype.eth_signTransaction = function(tx, callback) {
  var result;
  var error;

  try {
    result = this.state.signTx(tx, tx.from);
  } catch (e) {
    error = e;
  }
...
GethApiDouble.prototype.personal_importRawKey = function(rawKey, password, callback) {
  var account = this.state.createAccount({ secretKey: utils.addHexPrefix(rawKey) });
  this.state.accounts[account.address.toLowerCase()] = account;
  this.state.personal_accounts[account.address.toLowerCase()] = true;
  this.state.account_passwords[account.address.toLowerCase()] = password;
  callback(null, account.address);
...
GethApiDouble.prototype.personal_decrypt = function(address, dataToSign, callback) {
  var result;
  var error;

  try {
    result = this.state.decrypt(address, dataToSign);
  } catch (e) {
    error = e;
  }
